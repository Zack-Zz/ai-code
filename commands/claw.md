...
Start an interactive AI agent session that persists conversation history to disk and optionally loads ai-code skill context.
...
2. Loads conversation history from `$AI_CODE_HOME/claw/{session}.md`
3. Optionally loads ai-code skill context from `CLAW_SKILLS` env var
...
Sessions are stored as Markdown files in `$AI_CODE_HOME/claw/`:
