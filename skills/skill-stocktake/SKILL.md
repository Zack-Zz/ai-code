...
| Path | Description |
|------|-------------|
| `$AI_CODE_HOME/skills/` | Global skills (all projects) |
...
**Results cache:** `$AI_CODE_HOME/skills/skill-stocktake/results.json`

## Quick Scan Flow

Re-evaluate only skills that have changed since the last run (5–10 min).

1. Read `$AI_CODE_HOME/skills/skill-stocktake/results.json`
2. Run: `bash $AI_CODE_HOME/skills/skill-stocktake/scripts/quick-diff.sh \
         $AI_CODE_HOME/skills/skill-stocktake/results.json`
   (Project dir is auto-detected from `$PWD/.claude/skills`; pass it explicitly only if needed)
3. If output is `[]`: report "No changes since last run." and stop
4. Re-evaluate only those changed files using the same Phase 2 criteria
5. Carry forward unchanged skills from previous results
6. Output only the diff
7. Run: `bash $AI_CODE_HOME/skills/skill-stocktake/scripts/save-results.sh \
         $AI_CODE_HOME/skills/skill-stocktake/results.json <<< "$EVAL_RESULTS"`

## Full Stocktake Flow

### Phase 1 — Inventory

Run: `bash $AI_CODE_HOME/skills/skill-stocktake/scripts/scan.sh`
...
```json
{
  ...
  "skills": {
    "skill-name": {
      "path": "$AI_CODE_HOME/skills/skill-name/SKILL.md",
      "verdict": "Keep",
...
- Evaluation is blind: the same checklist applies to all skills regardless of origin (ai-code, self-authored, auto-extracted)
