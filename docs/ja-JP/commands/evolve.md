...
または`AI_CODE_PLUGIN_ROOT`が設定されていない場合(手動インストール):
...
1. `$AI_CODE_HOME/homunculus/instincts/`からすべてのinstinctsを読み取る
...
3. 3以上の関連instinctsの各クラスターに対して:
   ...
   - 適切なファイルを生成
   - `$AI_CODE_HOME/homunculus/evolved/{commands,skills,agents}/`に保存
